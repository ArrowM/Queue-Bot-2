...
export class DisplaysOption extends CustomOption {
	...
	protected async getViaSelectMenu(inter: SlashInteraction, scopedDisplays: Collection<bigint, DbDisplay>): Promise<Collection<bigint, DbDisplay>> {
		// build menu
		const label = DisplaysOption.ID;
		const options = scopedDisplays.map(display => ({
		...
		const selectedDisplays = scopedDisplays.filter(display => displayIds.includes(display.id));
